...
def createPaymentVoucher(request):
    try:
        ...
        if(serializer.is_valid()):
            ...
            cursor.execute(f"EXEC [fiac].[uspAddPaymentVoucher] %s",(json.dumps(serializer.data),))
            cursor.close()
...
def updatePaymentVoucher(request):
    try:
        ...
        if(serializer.is_valid()):
            cursor = connections[request.user.cid.cid].cursor()
            cursor.execute(f"EXEC [fiac].[uspUpdatePaymentVoucher] %s",(json.dumps(serializer.data),))
            cursor.close()
            return Response(serializer.data)
        UNSUCCESSFUL_REQUEST['message'] = serializer.errors
        return Response(UNSUCCESSFUL_REQUEST, status=400)
...
def getPaymentVoucherById(request):
    try:
        cursor = connections[request.user.cid.cid].cursor()
        cursor.execute(f"exec [fiac].[uspGetPaymentVoucherById] %s", (json.dumps(request.data), ))
...
def getPaymentVoucherReport(request):
    try:
        ...
        if(serializer.is_valid()):
            cursor = connections[request.user.cid.cid].cursor()
            cursor.execute(f"EXEC [fiac].[uspGetPaymentVoucherRep] %s",(json.dumps(serializer.data),))
            json_data = ConvertToJson(cursor)
            cursor.close()
            return JsonResponse(json_data, safe=False)
        UNSUCCESSFUL_REQUEST['message'] = serializer.errors
...
def deletePaymentVoucher(request):
    try:
        cursor = connections[request.user.cid.cid].cursor()
        cursor.execute(f"exec [fiac].[uspDeletePaymentVoucher] %s", (json.dumps(request.data), ))
        cursor.close()
